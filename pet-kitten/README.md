# Kitten — a Pet Simulator X style pet for Roblox

A rigged and animated kitten pet, built for Roblox Studio. Chunky proportions, oversized head,
big glossy eyes, black bow — the PSX pet look, in the pink/black/white palette from the
reference art.
...
| `textures/Kitten_ALB.png` | 256×256 albedo atlas (1.6 KB) |
...
| | This model | Limit |
| --- | --- | --- |
| Triangles | 12,432 | 20,000 |
| Faces | 6,216 — **100% quads**, no n-gons | quads where possible |
| Vertices | 6,302 | |
| Non-manifold edges | 0 (watertight) | must be watertight |
| Materials | 1 | exactly 1 |
| Bone influences per vertex | 2 | 4 |
| Weights on root bone | none | none allowed |
| Size | 48.0 × 66.1 × 77.6 cm = **1.71 × 2.36 × 2.77 studs** | |
...
fakes squash by moving parts instead of scaling them. Keep to location/rotation keys if you add
clips.

**One material per mesh.** Colour therefore comes from a palette atlas — a texture of four flat
swatches, with each part's UVs pinned inside its own swatch. The eyes, blush, bow and highlights
are *geometry* mapped to a swatch, not painted texture detail, so the pet stays crisp at any
distance and still reads correctly even if the texture fails to load.

| Swatch | Hex | Used for |
| --- | --- | --- |
| Pink | `#F7B6CE` | body, head, ears, legs, tail |
| Hot pink | `#FF6FA5` | inner ears, blush, nose |
| Black | `#1A1A1A` | eyes, bow, mouth |
| White | `#FFFFFF` | eye highlights, muzzle, chest, paws, tail tip |
...
Useful things to change: `TARGET_HEIGHT_CM` and the part scales in `build_model()` for
proportions, `PALETTE` for colours, `BONE_LAYOUT` for the rig, and the `anim_*` functions for
motion. Rarity variants in the PSX mould (gold, rainbow) are close to free — swap the palette
hex values and re-run.

## Note on originality

This is an original creature designed in the *style* of Pet Simulator X pets — the proportions,
the finish, the big-eyed read at small on-screen size. It is not a copy of any specific pet from
that game, so it is safe to ship in your own experience.
